...
<ResponseField name="work_dir" type="const char*" required>
  Absolute path to the working directory for file push/pull and command execution.
</ResponseField>
<ResponseField name="log_overflow_dir" type="const char*">
  Absolute path to the log overflow directory. Nullable - when `nullptr`, derived as `<work_dir>/_logs`.
</ResponseField>
