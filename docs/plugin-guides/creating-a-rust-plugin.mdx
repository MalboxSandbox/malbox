...
artifact_dir = "/tmp/malbox/artifacts"    # where plugin writes output
stash_dir = "/tmp/malbox/stash"           # internal result stash spillover
log_dir = "/tmp/malbox/logs"              # log overflow files
...
Add a one-line `build.rs` to ensure cargo re-runs the macro when `plugin.toml` changes:
