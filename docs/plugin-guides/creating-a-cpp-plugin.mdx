...
work_dir = "C:\\malbox\\work"     # working directory (default: /tmp/malbox or C:\malbox\work)
...
All `[runtime]` fields are optional and have sensible defaults. The values are baked into the plugin binary at build time via the generated `malbox_runtime_config.hpp` header - they are not read from the environment at launch.
