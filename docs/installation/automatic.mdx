...
<Tabs>
  <Tab title="bash / zsh">
    ```bash
    bash <(curl -sSfL https://raw.githubusercontent.com/DualHorizon/malbox/dev/back-end/scripts/install.sh)
    ```
  </Tab>
  <Tab title="fish">
    ```fish
    bash (curl -sSfL https://raw.githubusercontent.com/DualHorizon/malbox/dev/back-end/scripts/install.sh | psub)
    ```
  </Tab>
  <Tab title="non-interactive">
    ...
    ```bash
    MALBOX_CHANNEL=nightly curl -sSfL https://raw.githubusercontent.com/DualHorizon/malbox/dev/back-end/scripts/install.sh | bash
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Daemon source">
    Download a prebuilt binary or compile from source. Compiling from source lets you choose
    exactly which virtualization providers and machine provisioners to include.
  </Step>
  <Step title="PostgreSQL setup">
    Connect to an existing PostgreSQL server or let Malbox set up a managed instance.
    The installer validates the connection before continuing.
  </Step>
