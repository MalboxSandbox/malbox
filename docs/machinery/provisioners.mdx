---
title: "Machine provisioning with Ansible and native runners"
description: "Use Malbox provisioners to install packages, deploy guest plugins, run configuration scripts, and snapshot analysis machines after allocation."
---
...
|------|----------|-------------|
| `--provisioner` | Yes | Provisioner type (e.g. `"ansible"`, `"native"`). |
...
Machines can be referenced by name or numeric ID. If omitted, an interactive fuzzy selector is shown.
...
When plugins are specified (either via `--plugins` on the CLI or `guest_plugins` in the pipeline), the Ansible provisioner resolves each plugin's `plugin.toml` manifest and injects a `guest_plugins` extra variable as a JSON array. Each entry includes:
...
The `native` provisioner is a no-op step used primarily as a snapshot checkpoint. It creates a snapshot of the machine in its current state without running any configuration.

```bash
malboxctl machine provision win10-dev --provisioner native --snapshot base
```
