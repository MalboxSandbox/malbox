...
#[command(
    ...
    after_help = "Examples:\n  \
                  ...
                  malboxctl machine get threatforge-test-2\n  \
                  malboxctl machine provision win10-dev --provisioner ansible --snapshot with-yara\n  \
                  malboxctl machine snapshot list win10-dev\n  \
                  malboxctl machine snapshot delete win10-dev with-yara"
)]
...
#[command(after_help = "Examples:\n  \
                  malboxctl machine provision win10-dev --provisioner ansible --snapshot with-yara\n  \
                  malboxctl machine provision win10-dev --provisioner ansible --config '{\"playbook\": \"setup.yml\"}'\n  \
                  malboxctl machine provision win10-dev --provisioner native --plugins yara capa")]
...
async fn list(api: &ApiClient) -> Result<()> {
    ...
    if machines.is_empty() {
        format::empty_with_hint(
            "No machines found.",
            "Run 'malboxctl provider install' to set up a provider first.",
        );
        return Ok(());
    }
